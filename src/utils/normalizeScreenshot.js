...
async function normalizeRetinaScreenshot(browser, screenDimensions, base64Screenshot) {
  ...
  if (isImageScaled) {
    const normalizedScreenshot = await scaleImage(
      base64Screenshot,
      1 / screenDimensions.getPixelRatio(),
    );
...
async function normalizeIOSScreenshot(browser, screenDimensions, base64Screenshot) {
  ...
  const isIpad =
    screenDimensions.getScreenHeight() === 1024 || screenDimensions.getScreenWidth() === 1024;
...
export default async function normalizeSreenshot(browser, screenDimensions, base64Screenshot) {
  ...
  if (screenDimensions.getPixelRatio() > 1) {
    normalizedScreenshot = await normalizeRetinaScreenshot(
      browser,
      screenDimensions,
  ...
  if (browser.isMobile && browser.isIOS) {
    normalizedScreenshot = await normalizeIOSScreenshot(
      browser,
      screenDimensions,
      normalizedScreenshot,
    );
